...
def save_hg_dataset(hg_list_train, hg_list_val, hg_list_test, folder_path):
    ...
    hg_list_global = {}
    hg_list_global["train"] = hg_list_train
    hg_list_global["val"] = hg_list_val
    hg_list_global["test"] = hg_list_test
    with open(folder_path + "hg_global.pkl", "wb") as f:
        pickle.dump(hg_list_global, f)
...
def generate_dataset(dataset_type, node_condition, num_train, num_val, num_test, num_layers,
    ...
    with contextlib.redirect_stderr(open(os.devnull, 'w')):
        if dataset_type == 'erdos':
            ...
            print(f"No dataset type is recognized, it will be loaded from the file {dataset_type}.pkl")
            with open(Path("./dataset_files") / f"{dataset_type}.pkl", "rb") as f:
                dataset = pickle.load(f)
            hg_list_train = dataset["train"]
            hg_list_val = dataset["val"]
            hg_list_test = dataset["test"]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--type', type=str, required=True, help="The type of hypergraphs to generate (from 'erdos', 'sbm_custom', 'hypertrees' and 'ego' or the name of the associated file)")
