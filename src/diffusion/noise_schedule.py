...
class MultiHotUniformTransition:
    """
    Transition bit-flip indépendante pour des arêtes représentées
    par un vecteur binaire de longueur e_bits (multi-hot).
    
    - X, y : on conserve la transition uniforme catégorielle.
    - E     : chaque bit suit un canal Bernoulli vers une Bernoulli de paramètre marginal.
    """
    def __init__(self,
                 x_classes: int,
                 e_bits:    int,
                 y_classes: int,
                 e_marginals: torch.Tensor = None):

        self.X_classes = x_classes
        self.y_classes = y_classes
        self.u_x = torch.ones(1, x_classes, x_classes) / x_classes
        self.u_y = torch.ones(1, y_classes, y_classes) / y_classes


        if e_marginals is None:
            self.e_marginals = torch.full((e_bits,), 0.5)
        else:
            assert e_marginals.numel() == e_bits
            self.e_marginals = e_marginals.clone().float()
        self.e_bits = e_bits
    ...
    def get_Qt(self, beta_t: torch.Tensor, device: torch.device):
        ...
        - X: (bs, Xc, Xc)
        - y: (bs, yc, yc)
        - E: pour chaque bit k, matrice 2×2 de transitions {0,1}→{0,1}
              shape (bs,   e_bits, 2, 2)
        """
        # beta_t : (bs, 1)
        bs = beta_t.size(0)
        beta = beta_t.to(device)  # (bs,1,1)

        q_x = beta.unsqueeze(1) * self.u_x.to(device) \
            + (1-beta.unsqueeze(1)) * torch.eye(self.X_classes, device=device).unsqueeze(0)
        q_y = beta.unsqueeze(1) * self.u_y.to(device) \
            + (1-beta.unsqueeze(1)) * torch.eye(self.y_classes, device=device).unsqueeze(0)

        p_inf = self.e_marginals.to(device).unsqueeze(0).expand(bs, -1)  # (bs, e_bits)

        p1_if0 = beta * p_inf                   # (bs, e_bits)

        p1_if1 = (1-beta) + beta * p_inf        # (bs, e_bits)

        p0_if0 = 1 - p1_if0
        p0_if1 = 1 - p1_if1

        Q_e = torch.stack([
            torch.stack([p0_if0, p1_if0], dim=-1),
            torch.stack([p0_if1, p1_if1], dim=-1)
        ], dim=-2)  # → (bs, e_bits, 2, 2)  
    ...
    def get_Qt_bar(self, alpha_bar_t: torch.Tensor, device: torch.device):
        """
        Qt_bar pour le bruit cumulé de 0→t :
        même formules en remplaçant beta par (1 - alpha_bar_t) dans les termes constants.
        """
        bs = alpha_bar_t.size(0)
        a = alpha_bar_t.to(device)  # (bs,1)

        q_x = a.unsqueeze(1) * torch.eye(self.X_classes, device=device).unsqueeze(0) \
            + (1-a.unsqueeze(1)) * self.u_x.to(device)
        q_y = a.unsqueeze(1) * torch.eye(self.y_classes, device=device).unsqueeze(0) \
            + (1-a.unsqueeze(1)) * self.u_y.to(device)

        p_inf = self.e_marginals.to(device).unsqueeze(0).expand(bs, -1)

        # P(e_t=1 | e_0=0) = (1-a)*p_inf,  P(e_t=1 | e_0=1) = a + (1-a)*p_inf
        p1_if0 = (1-a) * p_inf
        p1_if1 = a + (1-a) * p_inf
        p0_if0 = 1 - p1_if0
        p0_if1 = 1 - p1_if1
