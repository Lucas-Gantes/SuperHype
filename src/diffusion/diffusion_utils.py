...
def sample_multicat_features(probX, probE, node_mask):
    """
    The equivalent of sample_discrete_features, but for the multi-categorical case.
    """
    bs, n, _ = probX.shape
    _, _, _, e_bits = probE.shape
    # Noise X
    # The masked rows should define probability distributions as well
    probX[~node_mask] = 1 / probX.shape[-1]
    ...
    # The masked rows should define probability distributions as well
    inverse_edge_mask = ~(node_mask.unsqueeze(1) * node_mask.unsqueeze(2))
    diag_mask = torch.eye(n, dtype=torch.bool, device=probE.device) 
    diag_mask = diag_mask.unsqueeze(0).expand(bs, -1, -1)  # To prevent self-loops

    inv_edge_mask = inverse_edge_mask.unsqueeze(-1)  # → (bs, n, n, 1)
    diag_mask     = diag_mask.unsqueeze(-1)      # → (bs, n, n, 1)

    probE = probE.clone()
    probE[inv_edge_mask.expand_as(probE)] = 0.5
    probE[diag_mask.expand_as(probE)]     = 0.5

    # Sample E
    E_t = torch.bernoulli(probE)  # (bs, n, n, e_bits)
    triu_mask = torch.triu(torch.ones(n, n, dtype=torch.bool, device=probE.device), diagonal=1)
    triu_mask = triu_mask.unsqueeze(0).unsqueeze(-1).expand(bs, -1, -1, e_bits)
    upper = E_t * triu_mask
    E_sym = upper + upper.transpose(1, 2)

    return PlaceHolder(X=X_t, E=E_sym, y=torch.zeros(bs, 0).type_as(X_t))
...
def mask_distributions(true_X, true_E, pred_X, pred_E, node_mask, multicat=False):
    """
    ...
    row_X[0] = 1.
    if multicat:
        row_E = 0.5 * torch.ones(true_E.size(-1), dtype=true_E.dtype, device=true_E.device)
    else:
...
def mask_distributions_X(true_X, pred_X, node_mask, multicat=False):
    # multicat argument included for API consistency, not used for X
...
def mask_distributions_E(true_E, pred_E, node_mask, multicat=False):
    if multicat:
        row_E = 0.5 * torch.ones(true_E.size(-1), dtype=true_E.dtype, device=true_E.device)
    else:
        row_E = torch.zeros(true_E.size(-1), dtype=true_E.dtype, device=true_E.device)
...
def sample_discrete_feature_noise(limit_dist, node_mask, multicat=False):
    ...
    if not multicat:
        # categorical sampling per edge -> one-hot
        U_E = e_limit.flatten(end_dim=-2).multinomial(1).reshape(bs, n_max, n_max)
        U_E = U_E.type_as(long_mask)
        U_E = F.one_hot(U_E, num_classes=e_limit.shape[-1]).float()
    else:
        # independent Bernoulli per bit (multi-hot)
        U_E = torch.bernoulli(e_limit)
        U_E = U_E.type_as(long_mask)
        # U_E = F.one_hot(U_E, num_classes=e_limit.shape[-1]).float()
...
def split_edge_label_tensor(E):
    """
    A function to split the edge labels into multiple graphs in multilabels mode.
    """
    bs, n1, n2, d = E.shape
    assert n1==n2
    ...
    for i in range(d):
        E_list.append(torch.stack([1.-E[:,:,:,i], E[:,:,:,i]], dim=-1))
    
    return E_list
    

def split_edge_transition_tensor(QE):
    """
    A function to split the edge transition tensor into multiple graphs in multilabels mode.
    """
    bs, d, d1, d2 = QE.shape
    assert d1==2
    assert d2==2
...
def apply_multilabel_transition(E, QE):
    # E: (bs, n, n, d)
    # QE: (bs, d, 2, 2)

    # Develop E as a superposisition of labeled graphs
    E_multicat = torch.stack([1-E, E], dim=-1).unsqueeze(-2)  # (bs, n, n, d, 1, 2)

    # Apply the transition tensor to the edges
    probE = E_multicat @ QE.unsqueeze(1).unsqueeze(1) # (bs, n, n, d, 1, 2)
